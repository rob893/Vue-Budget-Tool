    ...
    <FdTabItem striped label="Budget Tool" name="a" class="content">
      <div v-if="budget.getMonthlyExpenses().length > 0">
        ...
        <FdTable
          ...
          :items="budget.getMonthlyExpenses()"
        >
            ...
            <FdTableRow @click="goToExpense(item.name)">
              ...
              <FdTableCell
                ><FdButton
                  type="negative"
                  state="normal"
                  @click.stop="budget.removeMonthlyExpense(item)"
                  >Delete</FdButton
                ></FdTableCell
              >
            </FdTableRow>
        ...
        <h2>
          Total Monthly Per Month: ${{ budget.getMonthlySum() }} (${{
            budget.getMonthlySum() * 12
          }}
          per year)
        </h2>
      </div>
      ...
      <div v-if="budget.getAnnualExpenses().length > 0">
        ...
        <FdTable
          striped
          :headers="budgetHeaders"
          :items="budget.getAnnualExpenses()"
        >
          <template slot="row" slot-scope="{ item }">
            <FdTableRow @click="goToExpense(item.name)">
              <FdTableCell>{{ item.name }}</FdTableCell>
              ...
              <FdTableCell
                ><FdButton
                  type="negative"
                  state="normal"
                  @click.stop="budget.removeAnnualExpense(item)"
                  >Delete</FdButton
                ></FdTableCell
              >
            </FdTableRow>
        ...
        <h2>
          Total Annual Per Year: ${{ budget.getAnnualSum() }} (${{
            (budget.getAnnualSum() / 12).toFixed(2)
          }}
          per month)
        </h2>
      </div>
      ...
      <hr />
      <h2>Annual Income: $75000</h2>
      <h2>Remaining Budget: ${{ budget.getRemainingBudget() }}</h2>
      <h2>
        Remaining Budget Per Month: ${{ budget.getRemainingMonthlyBudget() }}
      </h2>
      <h2>
        Remaining Budget Per Week: ${{ budget.getRemainingWeeklyBudget() }}
      </h2>
      <br />
      ...
      <FdModal title="Add Expense" :active.sync="showAddExpense">
        ...
        <FdFormSet>
          ...
          <FdFormItem label="Cost">
            <FdInput
              ...
              type="number"
              :state="costState === '' ? null : costState"
              v-model="newCost"
            />
          </FdFormItem>
          <FdFormItem label="Type">
            <FdSelect
              v-model="selectType"
              :state="typeState === '' ? null : typeState"
            >
        ...
        <template slot="actions">
          ...
          <FdButton @click="addExpense()" styling="emphasized"
            >Add Expense</FdButton
          >
        </template>
      </FdModal>
    </FdTabItem>
...
export default mixins(
  TestMixin,
  Vue as VueConstructor<Vue & { $refs: { test: string } }>
).extend({
  name: "app",
  ...
  methods: {
    ...
    addExpense(): void {
      ...
      if (this.selectType === "Annual") {
        this.budget.addAnnualExpense(
          new Expense(this.newName, Number(this.newCost), ExpenseType.Annual)
        );
      } else {
        this.budget.addMonthlyExpense(
          new Expense(this.newName, Number(this.newCost), ExpenseType.Monthly)
        );
      }
    ...
    resetAPIDataToDefault(): void {
      API.post("https://rwherber.com/APIs/BudgetTool/index.php", {
        reset: true,
      }).then((value) => {
